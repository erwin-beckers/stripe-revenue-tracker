  ...
  return (
    <div className="card p-8 space-y-5">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        {mode === "password" && (
          <div>
            <label className="block text-xs font-medium text-fg-muted mb-2 uppercase tracking-wider">Password</label>
            <input type="password" required={mode === "password"} value={password} onChange={e => setPassword(e.target.value)}
              className="w-full px-4 py-2.5 rounded-lg bg-bg-elevated border border-border focus:border-accent outline-none transition"
              placeholder="••••••••" />
            <a href="/auth/forgot-password" className="text-xs text-fg-muted hover:text-fg mt-2 inline-block">Forgot password?</a>
          </div>
        )}

        <button type="submit" disabled={pending} className="btn-primary w-full justify-center">
          {pending ? "Working..." : mode === "magic" ? "Email me a login link" : "Sign in"}
        </button>
      </form>
      ...
      <div className="text-center">
        <button type="button" onClick={() => setMode(mode === "password" ? "magic" : "password")}
          ...
          {mode === "password" ? "Or email me a link instead" : "Or use a password"}
        </button>
      </div>
      ...
      <p className="text-center text-sm text-fg-muted pt-2">
        New here? <a href="/signup" className="text-accent hover:text-accent-hover">Create an account</a>
      </p>
    </div>
  );
}
